...
Chromosom::Chromosom(Chromosom& parent1, Chromosom& parent2, int openStoragesLen,
	int userLen, int chooseType, int randNumber) {
...
void Chromosom::mutation() {
	...
	int max = rand() % ((int)(1 + representation.size() / 10.0));
...
void createChildren(std::vector<Chromosom>& children, Chromosom parent1, 
	Chromosom parent2, int chooseType) {

	int userLen = parent1.representation.size();

	children = { Chromosom(parent1.openStorages, userLen), 
	...
	if (chooseType == 0) {
		...
		for (int i = 0; i < userLen; i++) {
			...
			if (choose == 0) {
				children[0].representation.push_back(parent1.representation[i]);
				children[1].representation.push_back(parent2.representation[i]);
			} else {
				children[1].representation.push_back(parent1.representation[i]);
				children[0].representation.push_back(parent2.representation[i]);
			}
		}
